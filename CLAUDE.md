# Agentic Polyglot Starter - Claude Code Rules
...
Generic sub-agent definitions live in `.claude/agents/`:

| Agent | Scope |
|---|---|
| `app-dev` | `apps/`, `packages/` |
| `backend-dev` | `cmd/`, `services/`, `internal/`, `contracts/` |
| `docs-maintainer` | `docs/`, `AGENTS.md`, `CLAUDE.md` |
| `tester` | Tests and failing gates |
| `code-reviewer` | Read-only review before PRs |

Do not add local worktrees, agent memory, or session artifacts to Git.
